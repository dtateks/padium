...
- `ScrollSuppressor.isMultitouchActive` and `currentFingerCount` are set from `GestureEngine`'s pipeline task via the injected `MultitouchStateSink` (which runs on an arbitrary thread from OMS); both use the same `os_unfair_lock`-protected state read by the CGEventTap callback for scroll suppression and physical click routing
