...
| GestureSlot.swift | Enum | Gesture slots include 1-finger and 2-finger double-tap alongside swipe and higher-finger tap/double-tap slots; `displayName`/`sectionTitle` for UI grouping |
...
| SettingsView.swift | UI | Groups slots by `sectionTitle`, shows per-row conflict warnings + experimental tap caveat, sensitivity copy limited to swipes; copy mentions Smart Zoom among experimental tap conflicts |
...
- `GestureClassifier` applies a +20 point base boost before threshold mapping; swipe thresholds range 0.04-0.10 (previous 0.06-0.14), so default 50% behaves like the previous 70% calibration; shared sensitivity remaps swipe thresholds only and is read live without an AppState runtime restart
...
- `GestureEngine` ignores frames after a committed gesture until a lift frame clears the candidate; tap/double-tap recognition is experimental and now requires stable full-finger contact sets per raw frame before tap tracking, preferring drop over cross-finger-count misfire
