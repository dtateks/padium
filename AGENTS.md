...
- Classes that own OS-level resources (NotificationCenter observer tokens, Timer instances, CGEventTap + CFRunLoopSource, NSPanel, MultitouchSupport callbacks) MUST tear them down in `deinit`. For `@MainActor` classes the `deinit` runs nonisolated under Swift 6 strict concurrency — store the resource tokens as `private nonisolated(unsafe) var ...` so `deinit` can read them; AppKit-only APIs (e.g. `NSPanel.orderOut`) get dispatched to `DispatchQueue.main` from the nonisolated deinit. The bridge-listener cleanup chain (`MultitouchGestureSource.deinit → monitor.stopListening`) means dropping a runtime can release the trackpad subscriptions automatically without an explicit `stop()` call.
